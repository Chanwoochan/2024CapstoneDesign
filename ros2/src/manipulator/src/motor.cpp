...
void motor_position_control(unsigned char *data, unsigned char motor_id, unsigned short position)
{
    ...
    data[10] = position     & 0xFF;
    data[11] =(position>>8) & 0xFF;
    ...
    for(int i{};i<16;i++)
        {std::cout.width(2); std::cout.fill('0');std::cout << std::hex << (unsigned short)(data[i]);}
    std::cout << std::dec << "\n";
}
